...
export class SupabaseService {
  ...
  static async fetchTargetedReportData(params: {
    ...
    try {
      const apiRes = await ApiService.fetchTargetedReportData(params);
      return {
        success: apiRes.success,
        realisasi: apiRes.realisasi,
        workOrders: apiRes.workOrders,
        totalCount: apiRes.totalCount,
        source: apiRes.source === 'hypercloud' ? 'supabase' : 'dexie',
      };
    } catch (err) {
      console.warn('SupabaseService.fetchTargetedReportData error:', err);
      return {
        success: false,
        realisasi: [],
        workOrders: [],
