...
// We can test selecting columns: e.g. supabase.from(table).select('column_name')
// If column doesn't exist, PostgREST returns PGRST204: Could not find the 'xyz' column of 'table'
async function probeColumn(table: string, col: string): Promise<boolean> {
  ...
  if (error.code === 'PGRST204' || error.message.includes('Could not find')) return false;
  return false;
}

const candidateCols: Record<string, string[]> = {
  INISIASI: [
    'id', 'ID', 'Id', 'no', 'NO', 'No',
    'kodeUL', 'Kode_UL', 'kode_ul', 'KODE_UL', 'Kode', 'kode',
    'namaUL', 'Nama_UL', 'nama_ul', 'NAMA_UL', 'Nama', 'nama',
    'id_spreadsheet', 'idSpreadsheet', 'ID_SPREADSHEET', 'spreadsheet_id', 'spreadsheetId',
    'url_GAS', 'urlGas', 'URL_GAS', 'gas_url', 'gasUrl',
    'Folder_id_spreadsheet', 'folder_id_spreadsheet', 'folderIdSpreadsheet', 'folder_spreadsheet', 'drive_folder_id',
    'Folder_id_Foto', 'folder_id_foto', 'folderIdFoto', 'folder_foto', 'photo_folder_id',
    'Folder_id_absensi', 'folder_id_absensi', 'folderIdAbsensi', 'folder_absensi', 'absensi_folder_id',
    'created_at', 'createdAt', 'updated_at', 'updatedAt'
  ],
  WORK_ORDER: [
    'id', 'ID', 'Id', 'unitId', 'unit_id', 'UNIT_ID', 'UnitId',
    ...
    'lokasi', 'LOKASI', 'lokasi', 'Lokasi', 'alamat', 'ALAMAT',
  ...
  REALISASI: [
    ...
    'ulpName', 'NAMA_ULP', 'ULP', 'nama_ulp',
    'reguName', 'NAMA_REGU', 'REGU_ROW', 'REGU', 'regu_name',
    'penyulangName', 'NAMA_PENYULANG', 'PENYULANG', 'penyulang_name',
  ...
  USERS: [
    'id', 'ID', 'Id', 'unitId', 'unit_id', 'UNIT_ID', 'UnitId',
    'nip', 'NIP', 'nip', 'UserID', 'User_ID', 'user_id',
    'name', 'NAMA', 'Nama', 'Name',
  ...
  ULP: [
    'id', 'ID', 'Id', 'unitId', 'unit_id', 'UNIT_ID', 'UnitId',
    'kodeULP', 'Kode_ULP', 'kode_ulp', 'KODE_ULP', 'Kode', 'kode',
...
async function main() {
  for (const [table, cols] of Object.entries(candidateCols)) {
    const valid: string[] = [];
    for (const c of cols) {
      const exists = await probeColumn(table, c);
      if (exists) valid.push(c);
    }
    console.log(`=== TABLE ${table} VALID COLUMNS ===`);
